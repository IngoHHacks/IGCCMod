...
public class CardSelectSequencers
{
    ...
    public static IEnumerator CreateRarity(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        List<SelectableCard> cards = new();
        Transform obj = ((List<Transform>)__instance.GetType()
            .GetField("cardPositionMarkers", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(__instance))[0];

        for (int i = 0; i <= 2; i++)
        {
            Vector3 vector = GetCardIndexLoc(obj, i + 6);
            CardInfo c = ScriptableObject.CreateInstance<CardInfo>();
            CardModificationInfo addTo = new();
            switch (i)
            {
                case 0:
                    {
                        addTo.nameReplacement = "regular";
                        List<CardMetaCategory> cats = new()
                        {
                ...
                case 1:
                    {
                        ...
                        List<CardMetaCategory> cats = new() { CardMetaCategory.Rare };
                        c.metaCategories = cats;
            ...
            selectableCard.CursorSelectEnded = (Action<MainInputInteractable>)Delegate.Combine(
                selectableCard.CursorSelectEnded, (Action<MainInputInteractable>)delegate(MainInputInteractable c2)
                {
    ...
    public static IEnumerator CreateComplexity(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        for (int i = 0; i <= 3; i++)
        {
            ...
            CardInfo c = ScriptableObject.CreateInstance<CardInfo>();
            CardModificationInfo addTo = new() { nameReplacement = ((CardComplexity)i).ToString() };
        ...
        Singleton<TextDisplayer>.Instance.ShowMessage(
            "Choose the [c:bR]complexity[c:] of card. This determines when it is unlocked in the tutorial.");
        foreach (SelectableCard item in cards)
        {
            item.SetInteractionEnabled(true);
            item.SetEnabled(true);
    ...
    public static IEnumerator CreateCostCard(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        while (!valid)
        {
            ...
            List<CardInfo> choices = new();
            Transform obj = ((List<Transform>)__instance.GetType()
                .GetField("cardPositionMarkers", BindingFlags.NonPublic | BindingFlags.Instance)
                ...
                if ((page * 15) + i <= 23 || i == 15)
                {
                    if (i <= 14 || (i == 15 && page > 0) || (i == 16 && page < 1))
                    {
                        ...
                        if (i <= 14)
                        {
                            if ((page * 15) + i >= 21)
                            {
                                List<GemType> gems = new() { (GemType)((page * 15) + i - 21) };
                                addTo.addGemCost = gems;
                                addTo.nameReplacement = gems[0] + " gem";
                            }
                            ...
                            else if ((page * 15) + i >= 5)
                            {
                                ...
                                if ((page * 15) + i - 4 == 1)
                                {
                                    addTo.nameReplacement = "1 bone";
                                }
                                else
                                {
                                    addTo.nameReplacement = (page * 15) + i - 4 + " bones";
                                }
                            }
                            else if ((page * 15) + i >= 1)
                            {
                        ...
                        else
                        {
                            if (i == 15)
                            {
                                addTo.nameReplacement = "previous page";
                            }
                            else if (i == 16)
                            {
                                addTo.nameReplacement = "next page";
                            }
                        ...
                        SelectableCard selectableCard =
                            CreateCard(__instance, choices, cards, vector, true, true, false);
                        selectableCard.CursorSelectEnded = (Action<MainInputInteractable>)Delegate.Combine(
                            selectableCard.CursorSelectEnded,
                            (Action<MainInputInteractable>)delegate(MainInputInteractable c2)
                            {
                                selectedCard = (SelectableCard)c2;
                            });
                    }
    ...
    public static IEnumerator CreateCostCardAdditive(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        if (selectedCard.Info.BloodCost > 0)
        {
            preview.Info.cost += selectedCard.Info.BloodCost;
        }
        else if (selectedCard.Info.BonesCost > 0)
        {
            preview.Info.bonesCost += selectedCard.Info.BonesCost;
        }
        else if (selectedCard.Info.EnergyCost > 0)
        {
            preview.Info.energyCost += selectedCard.Info.EnergyCost;
        }
        else if (selectedCard.Info.GemsCost.Count > 0)
        {
            if (preview.Info.gemsCost == null)
            {
                preview.Info.gemsCost = new List<GemType>();
            }
        }
    ...
    public static IEnumerator CreatePortrait(DeathCardCreationSequencer __instance, SelectableCard preview,
        List<Texture2D> portraits, int type)
    {
        bool valid = false;
        int page = 0;
        SelectableCard selectedCard = null;
        ...
        while (!valid)
        {
            ...
            for (int i = 0; i <= 16; i++)
            {
                if ((page * 15) + i < portraits.Count || i == 15)
                {
                    if (i <= 14 || (i == 15 && page > 0) || (i == 16 && page < Math.Ceiling(portraits.Count / 15.0f)))
                    {
                        ...
                        {
                            // Set portrait
                            c.portraitTex = Sprite.Create(portraits[(page * 15) + i],
                                new Rect(0.0f, 0.0f, 114.0f, 94.0f), new Vector2(0.5f, 0.5f));
                            // Set name
                            string name = "";
                            if (portraits[(page * 15) + i].name.StartsWith("portrait_"))
                            {
                                name = portraits[(page * 15) + i].name.Substring(9).Replace("_", " ");
                            }
                            else
                            {
                                name = portraits[(page * 15) + i].name.Replace("_", " ");
                            }

                            if (name.Contains(Path.DirectorySeparatorChar.ToString()))
                            {
                                addTo.nameReplacement =
                                    name.Substring(name.LastIndexOf(Path.DirectorySeparatorChar) + 1);
                            }
                            else
                            {
                                addTo.nameReplacement = name;
                            }
                        }
                        else
                        {
            ...
            {
                if (page < Math.Ceiling(portraits.Count / 15.0f))
                {
                    page++;
                }
        ...
        if (type == 0)
        {
            preview.Info.portraitTex = Sprite.Create(selectedCard.Info.portraitTex.texture,
                new Rect(0.0f, 0.0f, 114.0f, 94.0f), new Vector2(0.5f, 0.5f));
        }
        else
        {
            preview.Info.alternatePortrait = Sprite.Create(selectedCard.Info.portraitTex.texture,
                new Rect(0.0f, 0.0f, 114.0f, 94.0f), new Vector2(0.5f, 0.5f));
        }
    ...
    public static IEnumerator CreateEvolution(DeathCardCreationSequencer __instance, SelectableCard preview, int type)
    {
        ...
        while (!valid)
        {
            ...
            for (int i = 0; i <= 16; i++)
            {
                ...
                {
                    if (i <= 14 || (i == 15 && page > 0) ||
                        (i == 16 && page < Math.Ceiling(validCards.Count + (1 / 15.0f))))
                    {
                        Vector3 vector = GetCardIndexLoc(obj, i);
                        CardInfo c = ScriptableObject.CreateInstance<CardInfo>();
                        CardModificationInfo addTo = new();
                        if (i <= 14)
                        {
                            if ((page * 15) + i == 0)
                            {
                                if (type == 0)
                                {
                                    addTo.nameReplacement = "default (+1/2)";
                                }
                                else if (type == 1)
                                {
                                    addTo.nameReplacement = "default (opposum)";
                                }
                            }
                            else
                            {
                                c = validCards[(page * 15) + i - 1];
                            }
                        ...
                        if ((page * 15) + i == 0)
                        {
                            selectableCard = CreateCard(__instance, choices, cards, vector, true, true, false);
                        }
                        else
                        {
                            selectableCard = CreateCard(__instance, choices, cards, vector, false, false, false);
                        }
            ...
            if (type == 0)
            {
                Singleton<TextDisplayer>.Instance.ShowMessage("Choose which card it will evolve into.");
            }
            else if (type == 1)
            {
                Singleton<TextDisplayer>.Instance.ShowMessage("Choose which card it will create upon death.");
            }
            ...
            {
                if (page < Math.Ceiling((validCards.Count + 1) / 15.0f))
                {
                    page++;
                }
        ...
        if (!selectedCard.Info.DisplayedNameEnglish.Equals("default (+1/2)") &&
            !selectedCard.Info.DisplayedNameEnglish.Equals("default (opposum)"))
        {
            if (type == 0)
            {
                preview.Info.evolveParams = new EvolveParams { evolution = selectedCard.Info, turnsToEvolve = 1 };
            }
            else if (type == 1)
            {
                preview.Info.iceCubeParams = new IceCubeParams { creatureWithin = selectedCard.Info };
            }
        }
    ...
    public static IEnumerator CreateEvolutionTurnCount(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        for (int i = 0; i <= 9; i++)
        {
            ...
            if (i == 0)
            {
                addTo.nameReplacement = "1 turn";
            }
            else
            {
                addTo.nameReplacement = i + 1 + " turns";
            }

            addTo.abilities.Add(Ability.Evolve);
            c.Mods.Add(addTo);
            c.evolveParams = new EvolveParams { turnsToEvolve = i + 1 };
            choices.Add(c);
            SelectableCard selectableCard = CreateCard(__instance, choices, cards, vector, true, true, false);
            selectableCard.CursorSelectEnded = (Action<MainInputInteractable>)Delegate.Combine(
                selectableCard.CursorSelectEnded, (Action<MainInputInteractable>)delegate(MainInputInteractable c2)
        ...
        Singleton<TextDisplayer>.Instance.ShowMessage(
            "[c:bR]How many turns[c:] should it take for this card to evolve?");
        foreach (SelectableCard item in cards)
        {
            item.SetInteractionEnabled(true);
            item.SetEnabled(true);
    ...
    public static IEnumerator CreateAttackCard(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        while (!valid)
        {
            ...
            for (int i = 0; i <= 16; i++)
            {
                ...
                {
                    if (i <= 14 || (i == 15 && page > 0) ||
                        (i == 16 && page < Math.Ceiling((9999 + specialStatIcons) / 15.0f)))
                    {
                        Vector3 vector = GetCardIndexLoc(obj, i);
                        CardInfo c = ScriptableObject.CreateInstance<CardInfo>();
                        CardModificationInfo addTo = new();
                        ...
                        choices.Add(c);
                        SelectableCard selectableCard =
                            CreateCard(__instance, choices, cards, vector, false, true, false);
                        selectableCard.CursorSelectEnded = (Action<MainInputInteractable>)Delegate.Combine(
                            selectableCard.CursorSelectEnded,
                            (Action<MainInputInteractable>)delegate(MainInputInteractable c2)
                            {
            ...
            {
                if (page < Math.Ceiling((9999 + specialStatIcons) / 15.0f))
                {
                    page++;
                }
            ...
            for (int i = 0; i <= 16; i++)
            {
                if ((page * 15) + i <= 9998 || i == 15)
                {
                    if (i <= 14 || (i == 15 && page > 0) || (i == 16 && page < 667))
                    {
                        ...
                        choices.Add(c);
                        SelectableCard selectableCard =
                            CreateCard(__instance, choices, cards, vector, true, false, false);
                        selectableCard.CursorSelectEnded = (Action<MainInputInteractable>)Delegate.Combine(
                            selectableCard.CursorSelectEnded,
                            (Action<MainInputInteractable>)delegate(MainInputInteractable c2)
                            {
    ...
    public static IEnumerator CreateSigilCard(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        int page = 0;
        SelectableCard selectedCard = null;
        List<Ability> selectedSigils = new();
        List<SelectableCard> cards = new();

        List<Ability> allAbilities = Enumerable.Range(0, (int)Ability.NUM_ABILITIES).Select(a => (Ability)a).ToList();

        List<Ability> abilities = allAbilities.Where(a =>
            AbilitiesUtil.GetInfo(a) != null &&
            (AbilitiesUtil.GetInfo(a).metaCategories.Contains(AbilityMetaCategory.Part1Rulebook) ||
             AbilitiesUtil.GetInfo(a).metaCategories.Contains(AbilityMetaCategory.Part3Rulebook) ||
             AbilitiesUtil.GetInfo(a).metaCategories.Contains(AbilityMetaCategory.Part1Modular) ||
             AbilitiesUtil.GetInfo(a).metaCategories.Contains(AbilityMetaCategory.Part3Modular))).ToList();

        List<FullAbility> moddedAbilities = AllAbilities;

        abilities.AddRange(moddedAbilities.Where(na =>
            na.Info != null && !allAbilities.Contains(na.Id) &&
            (na.Info.metaCategories.Contains(AbilityMetaCategory.Part1Rulebook) ||
             na.Info.metaCategories.Contains(AbilityMetaCategory.Part3Rulebook) ||
             na.Info.metaCategories.Contains(AbilityMetaCategory.Part1Modular) ||
             na.Info.metaCategories.Contains(AbilityMetaCategory.Part3Modular))).Select(na => na.Id));

        Transform objP = ((List<Transform>)__instance.GetType()
            .GetField("cardPositionMarkers", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(__instance))[0];

        // Create confirm card
        Vector3 vectorP = GetCardIndexLoc(objP, 18);
        CardInfo cP = ScriptableObject.CreateInstance<CardInfo>();
        CardModificationInfo addToP = new() { nameReplacement = "confirm" };
        cP.Mods.Add(addToP);
        ...
        confirm.CursorSelectEnded = (Action<MainInputInteractable>)Delegate.Combine(confirm.CursorSelectEnded,
            (Action<MainInputInteractable>) delegate(MainInputInteractable c2)
            {
                selectedCard = (SelectableCard)c2;
            });
        ...
        while (!valid)
        {
            ...
            if (!skipRegenerate)
            {
                ...
                obj = ((List<Transform>)__instance.GetType()
                    .GetField("cardPositionMarkers", BindingFlags.NonPublic | BindingFlags.Instance)
                    .GetValue(__instance))[0];
                for (int i = 0; i <= 16; i++)
                {
                    ...
                    {
                        if (i <= 14 || (i == 15 && page > 0) ||
                            (i == 16 && page < Math.Ceiling(abilities.Count / 15.0f)))
                        {
                            Vector3 vector = GetCardIndexLoc(obj, i);
                            CardInfo c = ScriptableObject.CreateInstance<CardInfo>();
                            CardModificationInfo addTo = new();
                            if (i <= 14)
                            {
                                List<Ability> abl = new() { abilities[(page * 15) + i] };
                                // Get name
                                if (AbilitiesUtil.GetInfo(abl[0]).rulebookName != null &&
                                    AbilitiesUtil.GetInfo(abl[0]).rulebookName != "")
                                {
                                    addTo.nameReplacement = "sigil of " + AbilitiesUtil.GetInfo(abl[0]).rulebookName;
                                }
                                else
                                {
                                    addTo.nameReplacement = "sigil of " + AbilitiesUtil.GetInfo(abl[0]).name;
                                }
                            ...
                            SelectableCard selectableCard = CreateCard(__instance, choices, cards, vector, true, true,
                                instantSpawn);
                            selectableCard.CursorSelectEnded = (Action<MainInputInteractable>)Delegate.Combine(
                                selectableCard.CursorSelectEnded,
                                (Action<MainInputInteractable>)delegate(MainInputInteractable c2)
                ...
                Singleton<TextDisplayer>.Instance.ShowMessage(
                    "Now choose some cards from which we will extract the [c:bR]sigils[c:]. Note that some may not work correctly.");
                foreach (SelectableCard item in cards)
                {
                    item.SetInteractionEnabled(true);
                    item.SetEnabled(true);
            ...
            confirm.SetInteractionEnabled(true);
            ...
            {
                if (page < Math.Ceiling(abilities.Count / 15.0f))
                {
                    page++;
                }
        ...
        Object.Destroy(confirm.gameObject, 2f);
    ...
    public static IEnumerator CreateSpecialAbilities(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        int page = 0;
        SelectableCard selectedCard = null;
        List<SpecialTriggeredAbility> selectedSAs = new();
        ...
        List<SpecialTriggeredAbility> validAbilities = new();
        for (int i = 1; i < abilitiesCount; i++)
        {
            if ((SpecialTriggeredAbility)i != SpecialTriggeredAbility.Ant &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.BellProximity &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.CardsInHand &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.Mirror &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.Lammergeier &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.GreenMage &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.EMPTY3 &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.EMPTY4 &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.EMPTY5 &&
                (SpecialTriggeredAbility)i != SpecialTriggeredAbility.EMPTY6)
            {
        ...
        while (!valid)
        {
            ...
            if (!skipRegenerate)
            {
                ...
                for (int i = 0; i <= 16; i++)
                {
                    ...
                    {
                        if (i <= 14 || (i == 15 && page > 0) ||
                            (i == 16 && page < Math.Ceiling((validAbilities.Count + traitCount) / 15.0f)))
                        {
                            Vector3 vector = GetCardIndexLoc(obj, i);
                            CardInfo c = ScriptableObject.CreateInstance<CardInfo>();
                            CardModificationInfo addTo = new();
                            if (i <= 14)
                            {
                                if ((page * 15) + i >= validAbilities.Count)
                                {
                                    ...
                                    addTo.nameReplacement =
                                        ((Trait)((page * 15) + i - validAbilities.Count + 1)).ToString();
                                }
                                else
                                {
                                    List<SpecialTriggeredAbility> abl = new() { validAbilities[(page * 15) + i] };
                ...
                Singleton<TextDisplayer>.Instance.ShowMessage(
                    "Finally, choose the hidden [c:bR]special abilities[c:] and [c:bR]traits[c:]. Again, some may not function properly.");
                foreach (SelectableCard item in cards)
                {
                    item.SetInteractionEnabled(true);
                    item.SetEnabled(true);
            ...
            {
                if (page < Math.Ceiling((validAbilities.Count + traitCount) / 15.0f))
                {
                    page++;
                }
            ...
            {
                if ((selectedCard.Info.SpecialAbilities.Count > 0 &&
                     selectedSAs.Contains(selectedCard.Info.SpecialAbilities[0])) ||
                    (selectedCard.Info.traits.Count > 0 && selectedTraits.Contains(selectedCard.Info.traits[0])))
                {
                    if (selectedCard.Info.SpecialAbilities.Count > 0)
                    {
                    ...
                    {
                        selectedTraits.Remove(selectedCard.Info.traits[0]);
                    }
                ...
                else
                {
                    ...
                    {
                        selectedTraits.Add(selectedCard.Info.traits[0]);
                    }
        ...
        {
            SetAttackAbility(preview.Info.specialStatIcon, preview.Info);
        }
    ...
    public static IEnumerator CreateTribeCard(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        while (!valid)
        {
                    ...
                    if ((page * 15) + i < tribeCount - 1 || i == 15)
                    {
                        if (i <= 14 || (i == 15 && page > 0) || (i == 16 && page < Math.Ceiling(tribeCount / 15.0f)))
                        {
                            ...
                            if (i <= 14)
                            {
                                ...
                                {
                                    List<Tribe> trb = new() { (Tribe)((page * 15) + i + 1) };
                                    addTo.nameReplacement = (Tribe)((page * 15) + i + 1) + " tribe";
                                    c.tribes = trb;
                                }
                                else
                                {
                                    TribeManager.TribeInfo ti =
                                        TribeManager.tribes[(page * 15) + i + 1 - (int)Tribe.NUM_TRIBES];
                                    Tribe t = ti.tribe;
                                    List<Tribe> trb = new() { t };
            ...
            {
                if (page < Math.Ceiling(tribeCount / 15.0f))
                {
                    page++;
                }
    ...
    public static IEnumerator PostCardSelect(List<SelectableCard> cards, SelectableCard selectedCard, int mode)
    {
        ...
        {
            item2.SetInteractionEnabled(false);
            item2.SetEnabled(false);
            if (item2 != selectedCard)
            {
                item2.Anim.PlayDeathAnimation();
                Object.Destroy(item2.gameObject, 2f);
            }
        }
        ...
        {
            case 0:
                if (selectedCard.Info.GemsCost.Count > 0)
                {
                    examineDialogue = string.Format("A cost of a [c:bR]{0} gem[c:].",
                        selectedCard.Info.GemsCost[0].ToString());
                }
                else if (selectedCard.Info.EnergyCost > 0)
                {
                    examineDialogue = string.Format("A cost of [c:bR]{0} energy[c:].", selectedCard.Info.EnergyCost);
                }
                else if (selectedCard.Info.BonesCost > 1)
                {
                    examineDialogue = string.Format("A cost of [c:bR]{0} bones[c:].", selectedCard.Info.BonesCost);
                }
                else if (selectedCard.Info.BonesCost > 0)
                {
                    examineDialogue = "A cost of [c:bR]1 bone[c:].";
                }
                else if (selectedCard.Info.BloodCost > 0)
                {
                    examineDialogue = string.Format("A cost of [c:bR]{0} blood[c:].", selectedCard.Info.BloodCost);
                }
                else
                {
                    examineDialogue = "A cost of... [c:bR]free[c:].";
                }

                break;
            case 1:
                if (selectedCard.Info.SpecialStatIcon != SpecialStatIcon.None)
                {
                    examineDialogue = string.Format("A special [c:bR]Power of {0}[c:].",
                        selectedCard.Info.DisplayedNameEnglish);
                }
                else
                {
                    examineDialogue = string.Format("[c:bR]{0} Power[c:].", selectedCard.Info.Attack);
                }
            ...
            case 5:
                examineDialogue =
                    string.Format("A complexity of [c:bR]{0}[c:].", selectedCard.Info.DisplayedNameEnglish);
                break;
            case 6:
                examineDialogue =
                    string.Format("An evolution to [c:bR]{0}[c:].", selectedCard.Info.DisplayedNameEnglish);
                break;
            case 7:
                if (selectedCard.Info.evolveParams.turnsToEvolve == 1)
                {
                    examineDialogue =
                        string.Format("[c:bR]{0} turn[c:].", selectedCard.Info.evolveParams.turnsToEvolve);
                }
                else
                {
                    examineDialogue =
                        string.Format("[c:bR]{0} turns[c:].", selectedCard.Info.evolveParams.turnsToEvolve);
                }
        ...
        IGCC.LookDown();
        selectedCard.Anim.PlayDeathAnimation();
        Object.Destroy(selectedCard.gameObject, 2f);
        yield return new WaitForSeconds(0.4f);
    }

    /// <see cref="PostCardSelect" />
    public static IEnumerator PostMultiSACardSelect(List<SelectableCard> cards,
        List<SpecialTriggeredAbility> selectedSAs, List<Trait> selectedTraits)
    {
        foreach (SelectableCard item2 in cards)
        {
            item2.SetInteractionEnabled(false);
            item2.SetEnabled(false);
            if (!((item2.Info.SpecialAbilities.Count > 0 && selectedSAs.Contains(item2.Info.SpecialAbilities[0])) ||
                  (item2.Info.traits.Count > 0 && selectedTraits.Contains(item2.Info.traits[0]))))
            {
                item2.Anim.PlayDeathAnimation();
                Object.Destroy(item2.gameObject, 2f);
            }
        ...
        {
            if (item2 != null &&
                ((item2.Info.SpecialAbilities.Count > 0 && selectedSAs.Contains(item2.Info.SpecialAbilities[0])) ||
                 (item2.Info.traits.Count > 0 && selectedTraits.Contains(item2.Info.traits[0]))))
            {
                item2.Anim.PlayDeathAnimation();
                Object.Destroy(item2.gameObject, 2f);
    ...
    public static IEnumerator PostMultiSigilCardSelect(List<SelectableCard> cards, List<Ability> selectedSigils)
    {
        ...
        if (selectedSigils.Count > 0)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                if (AbilitiesUtil.GetInfo(selectedSigils[i]).rulebookName != null &&
                    AbilitiesUtil.GetInfo(selectedSigils[i]).rulebookName != "")
                {
                    args += AbilitiesUtil.GetInfo(selectedSigils[i]).rulebookName;
                }
                else
                {
                    args += "sigil of " + AbilitiesUtil.GetInfo(selectedSigils[i]).name;
                }
    ...
    public static void DestroyAllCards(List<SelectableCard> cards, bool playDeathAnim)
    {
        foreach (SelectableCard item2 in cards)
        {
            ...
            {
                Object.Destroy(item2.gameObject, 0f);
            }
        }
    }
    ...
    public static Vector3 GetCardIndexLoc(Transform obj, int i)
    {
        ...
        if (i == 15)
        {
            vector = obj.position + new Vector3((Random.value * 0.05f) - 2.075f, 0f, (Random.value * 0.05f) + 3.0f);
        }
        else if (i == 16)
        {
            vector = obj.position + new Vector3((Random.value * 0.05f) + 6.925f, 0f, (Random.value * 0.05f) + 3.0f);
        }
        else if (i == 17)
        {
            vector = obj.position + new Vector3((Random.value * 0.05f) - 2.075f, 0f, (Random.value * 0.05f) + 5.0f);
        }
        else if (i == 18)
        {
            vector = obj.position + new Vector3((Random.value * 0.05f) + 6.925f, 0f, (Random.value * 0.05f) + 5.0f);
        }
        else if (i == 19)
        {
            vector = obj.position + new Vector3((Random.value * 0.05f) - 2.075f, 0f, (Random.value * 0.05f) + 1.0f);
        }
        else if (i == 20)
        {
            vector = obj.position + new Vector3((Random.value * 0.05f) + 6.925f, 0f, (Random.value * 0.05f) + 1.0f);
        }
        else
        {
            int z = i / 5;
            int x = i % 5;
            vector = obj.position + new Vector3((Random.value * 0.05f) + (x * 1.5f) - 0.575f, 0f,
                (Random.value * 0.05f) + 5.0f - (z * 2.0f));
        }

        return vector;
    }

    public static SelectableCard CreateCard(DeathCardCreationSequencer __instance, List<CardInfo> choices,
        List<SelectableCard> cards, Vector3 vector, bool hiddenAttack, bool hiddenHealth, bool instantSpawn)
    {
        // Invoke DeathCardCreationSequencer.SpawnCard()
        SelectableCard selectableCard = (SelectableCard)__instance.GetType()
            .GetMethod("SpawnCard", BindingFlags.NonPublic | BindingFlags.Instance)
            .Invoke(__instance, new object[] { __instance.transform });
        ...
        // Place at vector
        selectableCard.transform.position = vector + (Vector3.up * 2f) + ((Vector3)__instance.GetType()
            .GetField("THROW_CARDS_FROM", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(__instance) * 2f);
        selectableCard.transform.eulerAngles = new Vector3(90, 0, 0);
        if (instantSpawn)
        {
            Tween.Position(selectableCard.transform, vector, 0.0f, 0f);
        }
        else
        {
            Tween.Position(selectableCard.transform, vector, 0.1f, 0f);
        }
    ...
    public static IEnumerator FinalizeCard(DeathCardCreationSequencer __instance, SelectableCard preview)
    {
        ...
        if (selectedCard.Info.DisplayedNameEnglish.Contains("export "))
        {
            ...
            File.WriteAllBytes(Paths.PluginPath + "/IGCCExports/Artwork/" + name + ".png",
                CloneTextureReadable(preview.Info.portraitTex.texture).EncodeToPNG());
            Texture2D emission = PortraitLoader.Instance.GetEmissionForPortrait(preview.Info.portraitTex.texture);
            if (emission != null)
            {
                File.WriteAllBytes(Paths.PluginPath + "/IGCCExports/Artwork/" + name + "_emission.png",
                    CloneTextureReadable(emission).EncodeToPNG());
            }

            if (preview.Info.alternatePortrait != null)
            {
                File.WriteAllBytes(Paths.PluginPath + "/IGCCExports/Artwork/" + name + "_alt.png",
                    CloneTextureReadable(preview.Info.alternatePortrait.texture).EncodeToPNG());
                Texture2D altEmission =
                    PortraitLoader.Instance.GetEmissionForPortrait(preview.Info.alternatePortrait.texture);
                if (altEmission != null)
                {
                    File.WriteAllBytes(Paths.PluginPath + "/IGCCExports/Artwork/" + name + "_alt_emission.png",
                        CloneTextureReadable(altEmission).EncodeToPNG());
                }
            }
            ...
            yield return Singleton<TextDisplayer>.Instance.ShowUntilInput(
                "The card has been created, loaded, and exported to IGCCExports inside the BepInEx plugins folder.");
        }

        if (selectedCard.Info.DisplayedNameEnglish.Contains("quit"))
        {
            preview.Anim.PlayDeathAnimation();
            Object.Destroy(preview.gameObject, 0.5f);
            DestroyAllCards(cards, true);
    ...
    {
        RenderTexture renderTex = RenderTexture.GetTemporary(source.width, source.height, 0,
