...
[BepInDependency("MADH.inscryption.JSONLoader")]
public class IGCC : BaseUnityPlugin
{
    ...
    {
        bool skipMove = false;
        if (Singleton<ViewManager>.Instance.CurrentView == View.SanctumFloorDown)
        {
            skipMove = true;
        }

        Singleton<ViewManager>.Instance.SwitchToView(View.SanctumFloorDown, false, true);
        Tween.LocalRotation(Singleton<ViewManager>.Instance.CameraParent, new Vector3(90, 0, 0), 0.25f, 0f,
            Tween.EaseInOut);
        if (!skipMove)
        {
            Vector3 pos = Singleton<ViewManager>.Instance.CameraParent.localPosition;
            Tween.LocalPosition(Singleton<ViewManager>.Instance.CameraParent,
                new Vector3(pos.x, pos.y + 3.25f, pos.z + 4.25f), 0.25f, 0f, Tween.EaseInOut);
        }
    }
    ...
    public static void LookUp()
    {
        ...
        Singleton<ViewManager>.Instance.SwitchToView(View.SanctumFloorUp, false, true);
    }

    private static void ZoomIn()
    {
        Vector3 pos = Singleton<ViewManager>.Instance.CameraParent.localPosition;
        Tween.LocalPosition(Singleton<ViewManager>.Instance.CameraParent, new Vector3(pos.x, pos.y - 3.0f, pos.z),
            0.25f, 0f, Tween.EaseInOut);
    }

    private static void MoveDown()
    {
        Vector3 pos = Singleton<ViewManager>.Instance.CameraParent.localPosition;
        Tween.LocalPosition(Singleton<ViewManager>.Instance.CameraParent, new Vector3(pos.x, pos.y, pos.z - 1.25f),
            0.25f, 0f, Tween.EaseInOut);
    }
    ...
    private static void ResetCamera()
    {
        Vector3 pos = Singleton<ViewManager>.Instance.CameraParent.localPosition;
        Tween.LocalPosition(Singleton<ViewManager>.Instance.CameraParent,
            new Vector3(pos.x, pos.y + 3.0f, pos.z + 1.25f), 0.25f, 0f, Tween.EaseInOut);
    }
    ...
    public class PostSanctumIntroPatch : SanctumSceneSequencer
    {
        public static IEnumerator Postfix(IEnumerator __result, SanctumSceneSequencer __instance)
        {
            ...
            __instance.GetType().GetMethod("StartHumLoop", BindingFlags.NonPublic | BindingFlags.Instance)
                .Invoke(__instance, null);
            __instance.GetType().GetMethod("InitializeLeshy", BindingFlags.NonPublic | BindingFlags.Instance)
                .Invoke(__instance, null);
            Singleton<ViewManager>.Instance.SwitchToView(View.SanctumFloorDown, true, true);
            yield return new WaitForSeconds(0.5f);
            LookUp();
            Singleton<InteractionCursor>.Instance.SetHidden(false);
            yield return new WaitForSeconds(0.5f);
            yield return Singleton<TextDisplayer>.Instance.ShowUntilInput(
                "You entered the [c:bR]card creation[c:] mode.");
            yield return __instance.GetType()
                .GetMethod("DeathCardSequence", BindingFlags.NonPublic | BindingFlags.Instance)
                .Invoke(__instance, null);
            //}
        }
    }
    ...
    [HarmonyPatch(typeof(ViewManager), "OffsetPosition", typeof(Vector3), typeof(float))]
    public class RuleBookCameraPatch : ViewManager
    {
        public static bool Prefix(ViewManager __instance, Vector3 cameraOffset)
        {
            if (Instance.CurrentView == View.SanctumFloorDown)
            {
                ...
                if (cameraOffset.z >= 0)
                {
                    Tween.LocalRotation(Instance.CameraParent, new Vector3(90, 0, 0), 0.25f, 0f, Tween.EaseInOut);
                }
                else
                {
                    Tween.LocalRotation(Instance.CameraParent, new Vector3(105, 0, 0), 0.25f, 0f, Tween.EaseInOut);
                }
    ...
    {
        public static IEnumerator Postfix(IEnumerator __result, SanctumSceneSequencer __instance,
            DeathCardCreationSequencer ___deathCardSequencer)
        {
            //if (SaveManager.SaveFile.currentRun.causeOfDeath.opponentType == Opponent.Type.PixelP03FinaleBoss)
            //{
    ...
    [HarmonyPatch(typeof(DeathCardCreationSequencer), "CreateCardSequence", typeof(bool))]
    ...
    {
        public static IEnumerator Postfix(IEnumerator __result, DeathCardCreationSequencer __instance,
            KeyboardInputHandler ___keyboardInput)
        {
            //if (SaveManager.SaveFile.currentRun.causeOfDeath.opponentType == Opponent.Type.PixelP03FinaleBoss)
            //{
            bool done = false;
            if (SaveManager.SaveFile.currentRun.causeOfDeath.opponentType != Opponent.Type.PixelP03FinaleBoss)
            {
                ...
                yield return Singleton<TextDisplayer>.Instance.ShowUntilInput(
                    "Only worthy challengers may access the Card Creator.");
                yield return Singleton<TextDisplayer>.Instance.ShowUntilInput(
                    "Try again, but this time open the backroom door normally.");
                ...
                AudioSource audioSource2 =
                    AudioController.Instance.PlaySound2D("camera_flash_gameover", MixerGroup.None, 0.85f);
                audioSource2.gameObject.name = "flashSound";
                DontDestroyOnLoad(audioSource2.gameObject);
                AudioController.Instance.StopAllLoops();
                Singleton<UIManager>.Instance.Effects.GetEffect<ScreenColorEffect>()
                    .SetColor(GameColors.Instance.nearWhite);
            ...
            else
            {
                while (!done)
                {
                    ...
                    CardModificationInfo addTo = new() { nameReplacement = "current card" };
                    c.Mods.Add(addTo);
                    SelectableCard preview = CreatePreviewCard(__instance, vector, c);

                    List<int> prevSelects = new();
                    int i = 1;
                    int j = 0;
                    while (i != -1)
                    {
                    ...
                    Tween.Position(preview.transform, vector2, 0.25f, 0f);
                    ...
                    yield return __instance.GetType()
                        .GetMethod("EnterNameForCard", BindingFlags.NonPublic | BindingFlags.Instance)
                        .Invoke(__instance, new object[] { preview, nameMod });
                    yield return Singleton<TextDisplayer>.Instance.ShowUntilInput("[c:bR]" +
                        nameMod.nameReplacement.Trim() + "[c:]?");
                    yield return new WaitForSeconds(0.25f);
        ...
        private static IEnumerator CardCreationStep(int i, List<int> prevSelects, DeathCardCreationSequencer __instance,
            SelectableCard preview)
        {
            ...
            switch (i)
            {
                ...
                case 5:
                    ...
                    preview.SetInfo(preview.Info);
                    yield return new WaitForSeconds(0.25f);
                    if (preview.Info.Abilities.Contains(Ability.Evolve) ||
                        preview.Info.Abilities.Contains(Ability.Transformer))
                    {
                        // Evolution
